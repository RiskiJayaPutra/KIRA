                        ...
                        @foreach($wishlists as $item)
                            ...
                            <div class="flex flex-col sm:flex-row border-4 border-stroke bg-secondary overflow-hidden group">
                                ...
                                <div class="w-full sm:w-1/5 flex sm:flex-col border-t-4 sm:border-t-0 sm:border-l-4 border-stroke shrink-0">
                                    <!-- Remove Button -->
                                    <button wire:click="removeWishlist({{ $item->id }})" class="flex-1 bg-tertiary text-headline font-black p-2 hover:bg-background transition-colors flex justify-center items-center gap-2 border-r-4 sm:border-r-0 sm:border-b-4 border-stroke">
                                        <span class="text-2xl">💔</span>
                                        <span class="sm:hidden">HAPUS</span>
                                    </button>
                                    <!-- View/Cart Button -->
                                    <a href="{{ route('product.detail', $item->variant->product->slug) }}" wire:navigate class="flex-1 bg-button-fomo text-headline font-black p-2 hover:bg-headline hover:text-button-fomo transition-colors flex justify-center items-center gap-2">
                                        <span class="text-2xl">👀</span>
